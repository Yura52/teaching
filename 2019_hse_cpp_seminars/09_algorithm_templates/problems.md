...
`comparator(10, 20)`  
Лучший способ проверить работоспособность - передать объект типа `Less` в `std::sort`, все должно корректно сортироваться.
...
```
Добавить поддержку любых итераторов, а не только вектора. Добавить возможность передавать компаратор любой природы, то есть что угодно, что можно звать вот так:  
`comparator(a, b)`
...
# 5.A
Если предыдущие задачи компилируются, а сигнатура функции из 4F похожа на апокалипсис, то поздравляю, вы овладели очень важным и постоянно применяющимся на практике навыком. Пятерка в дневник и в журнал.
